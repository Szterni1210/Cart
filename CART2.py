import numpy as np
#klasa CART w niej znajdują się wszystkie funkcje
class CART(object):
    #funkcja ze startowymi parametrami drzewka
    def __init__(self, tree='cls', criterion='gini', prune='depth', max_depth=4, min_criterion=0.05):
        ...
        self.n_samples = None
        self.threshold = None
        self.depth = 0       # wartosc do ktorej drzewko bdzie sie zwiekszac aczkolwiek w zaleznosci od min. i max wartosci probke moze sie zatrzymac drzewko wczesniej
        self.prune = prune
        ...
        self.tree = tree
#funkcja która wraz z gini i entropy odpowiada za zwiekszanie sie drzewka
    def _grow_tree(self, features, target, criterion='gini'):
        ...
        if criterion in {'gini', 'entropy'}:       #metryki ktore sa bardzo do sb podobne i nie zmienia nam drzewka znajduja dobro dla classifi i redukcje wartosci dla regression
...
#funkcja ktora odpowiada za wypisanie drzewka????
    def print_tree(self):
        self.root._show_tree(0, ' ')
#funkcja mowi nam co gdzie zostanie dopisane, lewo czy prawo, dalsze galezie drzewka mianowicie wraz z wartosciami
...
#funkcja zwraca array z predykcjami wartości???
    def predict(self, features):
        return np.array([self.root._predict(f) for f in features])
#funkcja zwraca wartosci przewidywane???
    def _predict(self, d):
        if self.feature != None:
            ...
            return self.label
#funkcja okreslajaca wartosc kryterium gini lub mse
        ...
        if criterion == 'gini':
            ...
            for c in np.unique(target):
                ...
                if p > 0.0:
                    entropy -= p * np.log2(p)
            return entropy
#funkcja ktora okresla nam co sie dzieje w zaleznosci od próbek minimalna moze spowodowac zatrzymaniie wzrostu drzewka maks jako max liczba obiektow oceniona jako potencjalny punkt podziału dla kazdej iteracji
    def _prune(self, method, max_depth, min_criterion, n_samples):
        ...
        if pruning is True:
            ...
            self.right = None
            self.feature = None
#funkcja ktora przedstawia drzewko dopisujac dany element z prawej lub lewej
...
#funkcja okreslajaca wymiar drzewka???
...
#przykład drzewka użytego do predykcji wartości + porównanie wartości naszego kodu ze scikit
...
#drzewko decyzyjne które utwierdza nas w dokładności podanych wartości + porównanie wartości naszego kodu ze scikit
